...
class EntitiesSqlServerPersistence(IdentifiableSqlServerPersistence, IEntitiesPersistence):
    ...
    def _define_schema(self):
        self._clear_schema()
        self._ensure_schema(
            "CREATE TABLE [entities] ([id] VARCHAR(32), [site_id] VARCHAR(32),"
            " [type] VARCHAR(15), [name] VARCHAR(50), [content] nvarchar(max))")
    ...
    def get_one_by_name(self, correlation_id: Optional[str], entity_name: str) -> Optional[EntityV1]:
        query = "SELECT * FROM " + self._quote_identifier(self._table_name) + " WHERE [name]=?"
